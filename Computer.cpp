...
/* ********************************************************************* 
    -computer can take shortest route?
    -take the diagonal until the opponent is infront of the
    -computer can always leave one piece on home position

    1) loop over pieces --> each piece has 4 moves to choose from
       X -validate if the move is playable
        -pick best one 
            -if piece can move into home position
                -tie breaker if more than one available position
                -aim for middle or corner piece
            -move forward first, unless a capture piece
                -move from home position that is worth less points first
            -if piece can capture another
            -move backwards
    always want to know what piece you're moving and where

    looping over each piece --> evaluating 4 moves determining best move
    loop over best move --> 4 all piece pick piece with best move

    help:
    computer can look at open pieces and have it block??
********************************************************************* */
Piece Computer::CMovePiece(Board& table){
